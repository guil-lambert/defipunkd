...
**List + detail pages live for all DeFiLlama protocols**, fully statically rendered from `data/defillama-snapshot.json` + `data/overlays/*.ts`, showing raw metadata + `unknown` missingness rows.
...
data/
  ...
  overlays/
    <slug>.ts                 # human-curated per-field overrides (Wikipedia-style); starts empty
...
- **`data/overlays/<slug>.ts`** — typed per-protocol partials. The overlay type is a strict subset of the protocol schema, so overlays cannot introduce unknown fields. Overlays override the snapshot on a per-field basis at merge time.
...
Merge happens once at module load (build time in Next.js server components). No DB, no async. Unknown slugs return `undefined`; overlays for slugs that no longer exist in the snapshot log a warning and are ignored.
...
- `[curated]` — value came from a `data/overlays/<slug>.ts` override
...
- `data/overlays/` directory (empty, typed)
...
### Overlay shape (`data/overlays/<slug>.ts`)

```ts
import type { Overlay } from "@defibeat/registry";
export default {
  // Each field is a strict subset of ProtocolSnapshot. Unknown keys fail typecheck.
  website: "https://example.com",   // overrides the snapshot value
  // github, audit_links, hallmarks, etc. permitted
} satisfies Overlay;
```
