...
Browser-tool URL allowlists (Claude.ai web_fetch, ChatGPT browser) refuse to fetch URLs that haven't appeared verbatim in conversation context — that includes URLs you generate from a template, even when the template is described in this prompt. The "Pre-built read-API surfacer URLs" block above is your seed surface: those URLs appear verbatim and the allowlist accepts them. Each surfacer page response additionally pre-executes the contract's zero-arg view methods and renders any address-typed return values as /address/{chainId}/0x… links in the response body. **Those rendered links also appear verbatim post-fetch, so the allowlist accepts them too** — you may follow them directly without paste-back. This means transitively-discovered addresses (owner(), admin(), implementation(), factory(), Safe owners visible on the surfacer page) ARE readable in-run when reached via the surfacer page chain.

Two cases still hit the allowlist wall — for these, the cross-run ratchet applies:

  1. Addresses surfaced ONLY by /api/contract/read or /api/safe/owners JSON responses. Those JSON bodies contain checksummed addresses but do not embed surfacer URLs, so a generated /address/{chainId}/{addr} URL is rejected by the allowlist. Workaround: fetch the parent contract's surfacer page first — it renders the same view-method results with crawl links, so the desired URL appears verbatim there.
  2. Addresses surfaced from non-defipunkd sources (block-explorer pages, GitHub, audit PDFs). Generated surfacer URLs are rejected; do not ask the user to paste them back — the JSON-only output rule (Rule 5) is binding.

For both residual cases:
  ...
  - Add a checklist-coded entry to unknowns[] for each on-chain read you couldn't perform. Example: "C4: emergency-multisig-X owners not read; address 0xABC discovered via getRoleAdmin JSON response, blocked by browser allowlist".
