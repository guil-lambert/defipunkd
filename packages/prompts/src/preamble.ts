...
Two cases still hit the allowlist wall — for these, the cross-run ratchet applies:
  ...
  2. Addresses surfaced from non-defipunkd sources (block-explorer pages, GitHub, audit PDFs). Generated surfacer URLs are rejected by the allowlist — record the address via the ratchet flow below; the JSON-only output rule (Rule 5) is binding.
