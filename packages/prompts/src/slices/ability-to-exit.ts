...
- E3. For each pause guard: identify the role holder (which address holds PAUSE_ROLE / GUARDIAN / etc.) and the maximum pause duration. Specifically check whether PAUSE_INFINITELY (or equivalent uncapped pause) is callable, and which actor can call it (single multisig vs governance vote). For role-holder reads use https://defipunkd.com/api/contract/read?chainId=<id>&address=0x...&method=hasRole(bytes32,address)&args=0x...,0x... or getRoleAdmin(bytes32). For "is currently paused" checks use method=paused() or method=isPaused(uint256) with the resume code as args. Cite the URL with &block=<n> in evidence[].
